...
for (function (a) {
        a.fn.qrcode = function (b) {
            ...
            return "string" == typeof b && (b = {
                ...
                var c, d, e, f, g, h, i, j, k, n, a = new QRCode(b.typeNumber, b.correctLevel);
                for (a.addData(b.text), a.make(), c = document.createElement("canvas"), c.width = b.width, c.height = b.height, d = c.getContext("2d"),b.src && (e = new Image(), e.src = b.src, e.onload = function () {
                    ...
                    }),b.str && (n = new Image(), n.src = '../static/img/bg.png', n.onload = function () {
                        a.addStr(b.str,b.fontSize,b.border,b.width,b.height,d)
        ...
        addStr:function(str,fontSize,boder,width,height,ctx){
            ctx.fillStyle = "#3a99e6";
            var len = this.getStrLenght(str,fontSize);
            var h = 2*fontSize + 4*boder;
            var w = fontSize + len +2*boder;
            var left = (width - w)/2;
            var top = (height - h)/2;
            var imgData=ctx.createImageData(w,h);
            for(var i=0;i<w;i++){
                for(var j=0;j<h;j++){
                    if((i>=boder && i<w-boder && j>=boder && j<2*boder) ||
                       (i>=boder && i<w-boder && j>=h-2*boder && j<h-boder) ||
                       (j>=boder && j<h-boder && i>=boder && i<2*boder)||
                       (j>=boder && j<h-boder && i>=w-2*boder && i<w-boder)
                    ){
                        imgData.data[((j*w+i)*4)+0]=58;
                        imgData.data[((j*w+i)*4)+1]=153;
                        imgData.data[((j*w+i)*4)+2]=230;
                        imgData.data[((j*w+i)*4)+3]=255;
                    }else{
                        imgData.data[((j*w+i)*4)+0]=255;
                        imgData.data[((j*w+i)*4)+1]=255;
                        imgData.data[((j*w+i)*4)+2]=255;
                        imgData.data[((j*w+i)*4)+3]=255;
                    }
                }
            }
            ...
            ctx.fillText(str,left+w/2 ,top+h/2+fontSize*3/8);
        },
        getStrLenght:function(str,fz){
            var realLength = 0; 
            var len = str.length; 
            var charCode = -1; 
            for(var i = 0; i < len; i++){ 
